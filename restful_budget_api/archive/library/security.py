...
def admin_required(func: F) -> F:
    ...
    def decorator(
        *args: Any, **kwargs: Any
    ) -> Union[F, Tuple[Dict[str, str], int]]:
        ...
        *args: Any, **kwargs: Any
        ...
        if not request.headers.get("Authorization"):
            return (
                {
                    "error": (
                        "Please provide an API key in the Authorization "
        ...
        api_keys = [
            api_key[0]
            for api_key in db_fetchall(sql="SELECT password FROM users")
        ]
        ...
        *args: Any, **kwargs: Any
        ...
        if func.__name__ != request.method.lower():
            return (
                {
                    "error": (
                        f"user {request.method} verb with {func.__name__} "
                        + "method"
                    )
                },
                405,
